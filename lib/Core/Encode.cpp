...
#if FORMULA_DEBUG||!FORMULA_DEBUG
	std::ofstream out_file("bitcode_trace.z3expr",std::ios_base::out|std::ios_base::app);
	out_file <<"\n"<<z3_solver<<"\n";
	out_file.close();
#endif
...
void Encode::check_if() {
	...
	for (unsigned i = 0; i < ifFormula.size(); i++) {
		...
		int branch = (int) ifFormula[i].first->inst->inst;
		//2: collect info from the two former executions.
		if (trace->Id >= 2 && runtimeData.isDeadBranch(branch)) {
#if BRANCH_INFO
			cerr << "NNo!\n";
#endif
			continue;
		...
		if (result == z3::sat) {
			...
			if (runtimeData.isDeadBranch(branch))
				runtimeData.removeFromDeadBranch(branch);
			runtimeData.addToLiveBranch(branch);
		} else {
			if (!runtimeData.isLiveBranch(branch))
				runtimeData.addToDeadBranch(branch);
		}
...
//source code
	printSourceLine("source_trace", trace->path);
}
...
//raw_fd_ostream out_to_file("./output_info/counterexample.txt", ErrorInfo, 2 & 0x0200);
	for (unsigned i = 0; i < size; i++) {
		...
		if (currEvent->isGlobal) {
			...
			stringstream ss;
			ss << m.eval(z3_ctx.bv_const(str.c_str(), 32));	//just for
			out_to_file << ss.str();
		}
		if (currEvent->isLocal) {
			...
			stringstream ss;
			ss << m.eval(z3_ctx.bv_const(str.c_str(), 32));
			out_to_file << ss.str();
		}
		out_to_file << "\n";
	}
...
void Encode::logStatisticInfo() {
	...
	string ErrorInfo;
	raw_fd_ostream out("./statistic.info", ErrorInfo, 0x0200);
...
//		cerr << "kQueryExpr : " << trace->kQueryExpr[i] <<"\n";
...
//			cerr << "constraint : " << ifFormula[i].second << "\n";
			...
			z3::expr temp = kq->getZ3Expr(trace->brSymbolicExpr[i]);
			z3_solver.add(temp);
		}
	}
}
...
//		cerr << setiosflags(ios::fixed) << val << "\n";
		...
		if (IntToPtrInst::classof(inst)) {
			...
			ret = z3_ctx.bv_val(val, 32);	//to pointer, the default is 32bit.
		} else {
